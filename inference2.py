...
class InferenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # My Note: self.vae_processor.preprocess does similar thing as I preprocess with previous model like catdmZeng2024: turn RGB, resize, convert to tensor, and normalize to range [-1, 1]. The only difference, but very important is the normalization range is [-1,1] instead of [0,1] if using torchvision.transforms.ToTensor. Will use `VaeImageProcessor` from now if the normalization range does not conflict with any pretrained network used by the model. 
        return {
            'index': idx,
...
class VITONHDTestDataset(InferenceDataset):
    def load_data(self):
        pair_txt = os.path.join(self.args.paired_data_path)
...
# My note: GaussianBlur mask only work well with paired-data output; for unpaired data, some part of the original cloth item will be blended with the new cloth item.
...
def repaint2(source_img, mask, target_img):
    """Poisson blend
    
    Args:
        source_img (TYPE): Description
